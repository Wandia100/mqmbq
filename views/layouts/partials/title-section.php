            ...
            <div class="d-inline-block dropdown">
                ...
                <div tabindex="-1" role="menu" aria-hidden="true" class="dropdown-menu dropdown-menu-right">
                    <ul class="nav flex-column">
                        ...
                        <?php } ?>  
                        <?php  if(Yii::$app->controller->id == 'report' && Yii::$app->controller->action->id == 'commissionsummary'){ ?>
                            <li class="nav-item"> 
                                <a href="<?= Url::to(['/report/exportcommissionsummary','from'=> isset($_GET['from'])?$_GET['from']:date("Y-m-d"),'to'=> isset($_GET['to'])?$_GET['to']:'','criterion'=> isset($_GET['criterion'])?$_GET['criterion']:''])?>"> Export</a>
                            </li>
                        <?php } ?>  
                        <?php  if(Yii::$app->controller->id == 'report' && Yii::$app->controller->action->id == 'showsummary'){ ?>
                            <li class="nav-item"> 
                                <a href="<?= Url::to(['/report/exportshowsummary','from'=> isset($_GET['from'])?$_GET['from']:'','to'=> isset($_GET['to'])?$_GET['to']:'','criterion'=> isset($_GET['criterion'])?$_GET['criterion']:''])?>"> Export</a>
                            </li>
                        <?php } ?> 
                        <?php  if(Yii::$app->controller->id == 'report' && Yii::$app->controller->action->id == 'dailyawarding'){ ?>
                            <li class="nav-item"> 
                                <a href="<?= Url::to(['/report/exportdailyawarding','from'=> isset($_GET['from'])?$_GET['from']:'','to'=> isset($_GET['to'])?$_GET['to']:'','criterion'=> isset($_GET['criterion'])?$_GET['criterion']:''])?>"> Export</a>
                            </li>
                        <?php } ?> 
                        <?php  if(Yii::$app->controller->id == 'report' && Yii::$app->controller->action->id == 'revenue'){ ?>
                            <li class="nav-item"> 
