<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\FinancialSummaries;

/**
 * FinancialSummariesSearch represents the model behind the search form of `app\models\FinancialSummaries`.
 */
class FinancialSummariesSearch extends FinancialSummaries
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'created_at', 'updated_at'], 'safe'],
            [['mpesa_today', 'mpesa_total', 'transaction_history_today', 'transaction_history_total'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params, $daily = false, $monthly = false, $from = null, $to = null)
    {
        $query = FinancialSummaries::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'mpesa_today' => $this->mpesa_today,
            'mpesa_total' => $this->mpesa_total,
            'transaction_history_today' => $this->transaction_history_today,
            'transaction_history_total' => $this->transaction_history_total,
            'created_at' => $this->created_at,
            'updated_at' => $this->updated_at,
        ]);

        $query->andFilterWhere(['like', 'id', $this->id]);
        $today     = date( 'Y-m-d' );
        $yesterday = date( 'Y-m-d', strtotime( '-1 day' ) );
        if ( $daily ) {
                $query->andWhere( "DATE(created_at)>= DATE('" . $yesterday . "')" );
                $query->andWhere( "DATE(created_at)<= DATE('" . $today . "')" );
        }
        if ( $monthly ) {
                $query->andWhere( "MONTH(created_at)= MONTH(CURDATE())" );
                $query->andWhere( "YEAR(created_at)= YEAR(CURDATE())" );
        }
        if ( $from != null && $to != null ) {
                $query->andWhere( "DATE(created_at)>= DATE('" . $from . "')" );
                $query->andWhere( "DATE(created_at)<= DATE('" . $to . "')" );
        }

        return $dataProvider;
    }
}
